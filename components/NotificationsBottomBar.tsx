...
export default function NotificationsBottomBar({ active, onChange }: Props) {
  ...
  const indexFromKey = React.useCallback((key: NotifKey): number => {
    switch (key) {
      case "friendRequests": return 0; // leftmost
      case "regular": return 1;       // middle
      case "all": return 2;           // rightmost
      default: return 0;
    }
  }, []);

  const propActiveIndex = React.useMemo(() => indexFromKey(active), [active, indexFromKey]);
  ...
  React.useEffect(() => { animateToIndex(propActiveIndex); }, [propActiveIndex, animateToIndex]);
  ...
  return (
    <View style={styles.container} pointerEvents="box-none">
      <View style={styles.row} pointerEvents="box-none">
        {/* Independent avatar circle aligned to content edge */}
        ...
        {/* Bottom bar with only three tabs */}
